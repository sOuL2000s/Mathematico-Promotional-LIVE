...
import { FaEdit, FaTrash, FaPlusSquare, FaYoutube, FaSortAlphaDown, FaSortAlphaUp } from 'react-icons/fa'; // Added sort icons
...
const AdminVideosPage = () => {
  ...
  const handleVideoDeleted = async (deletedVideoId) => {
    if (!window.confirm("Are you sure you want to delete this video?")) return;
    try {
      setLoading(true);
      await deleteDoc(doc(db, 'videos', deletedVideoId));
      setAllVideos(prevVideos => prevVideos.filter(v => v.id !== deletedVideoId));
      setEditingVideo(null);
  ...
  return (
    <div className="py-8 md:py-12 px-4">
      <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-light-text mb-8 md:mb-10 animate-fade-in-up">Manage Videos</h1>
      ...
      <section className="bg-medium-dark p-6 md:p-8 rounded-xl shadow-lg border border-secondary mb-8 md:mb-12 animate-fade-in-up animation-delay-100">
        <h2 className="text-2xl sm:text-3xl font-bold text-primary mb-6 md:mb-8">{editingVideo ? 'Edit Video' : 'Create New Video'}</h2>
        <button
          onClick={() => {
        ...
        >
          <FaPlusSquare className="mr-2" /> {editingVideo ? 'Cancel Edit / Create New' : 'Create New Video'}
        </button>
      ...
      <section className="animate-fade-in-up animation-delay-200">
        <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-light-text mt-8 md:mt-12 mb-4 md:mb-6">Existing Videos</h3>
            ...
            <table className="min-w-full bg-medium-dark table-auto border-collapse">
              <thead className="bg-dark-background">
                <tr className="text-light-text uppercase text-xs sm:text-sm leading-normal">
                  ...
                  <th className="py-3 px-4 sm:px-6 text-left border-b border-secondary">YouTube Link</th>
              ...
              <tbody className="text-secondary text-xs sm:text-sm font-light">
                {displayedVideos.length === 0 ? (
                  <tr>
                    <td colSpan="4" className="py-4 text-center text-gray-text">No videos found matching your criteria.</td>
                  </tr>
                ) : (
                  displayedVideos.map((video) => (
                    <tr key={video.id} className="border-b border-secondary hover:bg-dark-background transition-colors duration-150">
                      <td className="py-3 px-4 sm:px-6 text-left whitespace-nowrap">
                        <span className="font-medium text-light-text">{video.title}</span>
                      </td>
                      <td className="py-3 px-4 sm:px-6 text-left">
                        <a href={video.youtubeLink} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline inline-flex items-center text-xs sm:text-sm">
                          <FaYoutube className="mr-1" /> View on YouTube
                        </a>
                      </td>
                      <td className="py-3 px-4 sm:px-6 text-center whitespace-nowrap text-gray-text">{formatDate(video.createdAt)}</td>
