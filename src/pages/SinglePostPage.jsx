...
const SinglePostPage = () => {
  ...
  if (error) return <div className="container mx-auto py-12 px-4 min-h-screen"><ErrorDisplay message={error} /></div>;
  ...
  return (
    <div className="container mx-auto py-12 px-4 min-h-screen">
      <div className="bg-white p-8 rounded-lg shadow-xl border border-gray-100">
        ...
        <span className="text-sm text-primary font-semibold mb-3 uppercase inline-block">{post.category}</span>
        <h1 className="text-4xl md:text-5xl font-extrabold text-dark mb-4 leading-tight">
          {post.title}
        </h1>
        <div className="text-gray-500 text-md mb-6 flex items-center space-x-4">
        ...
        <div className="flex items-center space-x-6 border-t border-b border-gray-200 py-4 mb-8">
          <button
            onClick={handleLike}
            className={`flex items-center space-x-2 text-primary font-semibold hover:text-green-600 transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed`}
          ...
          <button
            onClick={handleShare}
            className="flex items-center space-x-2 text-secondary font-semibold hover:text-blue-600 transition-colors duration-300"
          >
        ...
        {/* Comments Section */}
        <div className="mt-12">
          <h2 className="text-3xl font-bold text-dark mb-6">Comments ({comments.length})</h2>
          {comments.length === 0 ? (
            <p className="text-gray-600 text-lg">No comments yet. Be the first to share your thoughts!</p>
          ) : (
            <div className="space-y-6">
              {comments.map(comment => (
                <div key={comment.id} className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                  <div className="flex items-center mb-2">
                    <p className="font-semibold text-dark mr-2">{comment.commenterName || 'Anonymous'}</p>
                    <span className="text-gray-500 text-sm">&bull; {formatDate(comment.timestamp)}</span>
                  </div>
